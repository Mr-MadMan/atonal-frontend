<template>
  <t-row :gutter="[16, 16]">
    <t-col :xs="6" :xl="3" v-for="(item, index) in panelList" :key="item.title">
      <t-card :bordered="false" :title="item.title" :style="{ height: '168px' }" :class="{ 'dashboard-item': true }">
        <!-- 'dashboard-item--main-color': index == 0  -->
        <div class="dashboard-item-top">
          <span :style="{ fontSize: `${resizeTime * 36}px` }">{{ item.number }}</span>
        </div>
        <div class="dashboard-item-left">
          <span v-if="index === 0" :style="{ marginTop: `-8px` }">
            <!-- id="moneyContainer"
...
export default {
  ...
  computed: {
    ...
    ...mapState('system', ['userStatistic']), // 这里需要用到主题色和主题模式的全局配置
  },
  watch: {
    brandTheme() {
    ...
    userStatistic(info) {
      if (Object.keys(info).length > 0) {
        this.panelList[0].number = info.total_user;
        this.panelList[1].number = info.total_today_login;
      }
    },
  },
  mounted() {
